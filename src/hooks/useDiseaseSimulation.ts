...
import type { Node, Network } from "./useNetworkGenerator"
...
export const useDiseaseSimulation = () => {
  ...
  const initializeSimulation = useCallback((network: Network, vaccinationRate: number): SimulationState => {
    const { nodes } = network
  ...
  const simulateDay = useCallback((state: SimulationState, params: SimulationParameters): SimulationState => {
    const { reproductionNumber, recoveryRate, incubationPeriod, infectiousPeriod, socialDistancing } = params
    ...
    // Update history
    const stateHistory = [...state.stateHistory, { day: newDay, ...stats }]
  ...
  const runSimulation = useCallback(
    (initialState: SimulationState, params: SimulationParameters, days: number): SimulationState => {
