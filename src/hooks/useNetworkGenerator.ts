...
export const useNetworkGenerator = () => {
  const generateBarabasiAlbertNetwork = useCallback((
    ...
    m0: number = 5, // Initial connected network size
    m: number = 3,  // Number of edges to attach from a new node to existing nodes
    ...
    const nodes: Node[] = Array(m0).fill(null).map((_, i) => ({
      id: i,
      state: 'susceptible',
      connections: [],
      position: {
