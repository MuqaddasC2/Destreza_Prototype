...
const Navigation: React.FC<NavigationProps> = ({ currentTab, onTabChange, simulationStarted }) => {
  const handleTabClick = (index: number) => (event: React.MouseEvent) => {
    onTabChange(event as any, index)
  }
  ...
  return (
    ...
    >
      <StyledTabs value={currentTab} onChange={onTabChange} aria-label="navigation tabs" centered>
        <StyledTab
          icon={
            <GlowingIcon
              animate={{
